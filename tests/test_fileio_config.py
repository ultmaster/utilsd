...
# copied from mmcv tests with modifications
import argparse
import json
import os
...
from utilsd.fileio import Config, DictAction, dump, load

data_path = osp.join(osp.dirname(__file__), 'assets/fileio_config')
...
def test_fromfile():
    for filename in ['a.py', 'a.b.py', 'b.json', 'c.yaml']:
        cfg_file = Path(data_path) / filename
        cfg = Config.fromfile(cfg_file)
        assert isinstance(cfg, Config)
        assert cfg.filename == cfg_file.as_posix()
...
def test_merge_from_multiple_bases():
    cfg_file = osp.join(data_path, 'l.py')
    cfg = Config.fromfile(cfg_file)
    assert isinstance(cfg, Config)
    assert cfg.filename == cfg_file
    # cfg.field
    ...
    with pytest.raises(KeyError):
        Config.fromfile(osp.join(data_path, 'm.py'))


@pytest.mark.skip(reason='Base variables seem not supported')
def test_base_variables():
    for file in ['t.py', 't.json', 't.yaml']:
        cfg_file = osp.join(data_path, file)
        cfg = Config.fromfile(cfg_file)
        assert isinstance(cfg, Config)
        assert cfg.filename == cfg_file
        # cfg.field
    ...
    cfg_file = osp.join(data_path, 'v.py')
    cfg = Config.fromfile(cfg_file)
    assert isinstance(cfg, Config)
    assert cfg.filename == cfg_file
...
def test_merge_recursive_bases():
    cfg_file = osp.join(data_path, 'f.py')
    cfg = Config.fromfile(cfg_file)
    assert isinstance(cfg, Config)
    assert cfg.filename == cfg_file
    # cfg.field
...
def test_merge_from_dict():
    ...
    cfg_file = osp.join(data_path, 's.py')
    cfg = Config.fromfile(cfg_file)
...
def test_merge_delete():
    cfg_file = osp.join(data_path, 'delete.py')
    cfg = Config.fromfile(cfg_file)
    # cfg.field
    assert cfg.item1 == [1, 2]
...
def test_merge_intermediate_variable():
    ...
    cfg_file = osp.join(data_path, 'i_child.py')
    cfg = Config.fromfile(cfg_file)
    # cfg.field
    assert cfg.item1 == [1, 2]
...
def test_fromfile_in_config():
    cfg_file = osp.join(data_path, 'code.py')
    cfg = Config.fromfile(cfg_file)
    # cfg.field
...
def test_dump_mapping():
    cfg_file = osp.join(data_path, 'n.py')
    cfg = Config.fromfile(cfg_file)
